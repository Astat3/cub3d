...
static int	parse_texture_line(t_parsing *parsing, char *line, int fd)
{
    if (ft_strncmp(line, "NO", 2) == 0
        ...
        set_texture(&parsing->no_texture, skip_spaces(line + 2), "NO", fd);
    else if (ft_strncmp(line, "SO", 2) == 0
        && (line[2] == ' ' || line[2] == '\t' || line[2] == '\0'))
        set_texture(&parsing->so_texture, skip_spaces(line + 2), "SO", fd);
    else if (ft_strncmp(line, "WE", 2) == 0
        && (line[2] == ' ' || line[2] == '\t' || line[2] == '\0'))
        set_texture(&parsing->we_texture, skip_spaces(line + 2), "WE", fd);
    else if (ft_strncmp(line, "EA", 2) == 0
        && (line[2] == ' ' || line[2] == '\t' || line[2] == '\0'))
        set_texture(&parsing->ea_texture, skip_spaces(line + 2), "EA", fd);
    else if (line[0] == 'F'
        && (line[1] == ' ' || line[1] == '\t' || line[1] == '\0'))
        set_color(&parsing->floor_color, skip_spaces(line + 1), "F", fd);
    else if (line[0] == 'C'
        && (line[1] == ' ' || line[1] == '\t' || line[1] == '\0'))
        set_color(&parsing->ceiling_color, skip_spaces(line + 1), "C", fd);
    else
        return (0);
    return (1);
}
